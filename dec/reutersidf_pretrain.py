...
def main(db, params):
    ...
    for i in xrange(n_layer):
        ...
        last_dim = dim
    with open('reutersidf_pt_net.prototxt', 'w') as fnet:
        dec.make_net(fnet, encoder_layers+decoder_layers[::-1])

    with open('reutersidf_ft_solver.prototxt', 'w') as fsolver:
        fsolver.write("""net: "reutersidf_pt_net.prototxt"
base_lr: {0}
lr_policy: "step"
gamma: 0.1
stepsize: {1}
...
def pretrain_main(db, params):
    ...
    for i in xrange(n_layer):
        ...
        with open('reutersidf_stack_net.prototxt', 'w') as fnet:
            dec.make_net(fnet, layers)

        with open('reutersidf_pt_solver.prototxt', 'w') as fsolver:
            fsolver.write("""net: "reutersidf_stack_net.prototxt"
                base_lr: {0}
                lr_policy: "step"
                gamma: 0.1
                stepsize: {1}
        ...
        mean, net = dec.extract_feature('reutersidf_stack_net.prototxt', model,
                                        [str_x], 1, train=True, device=0)

        net.save('reutersidf_stack_init.caffemodel')

        os.system('caffe train --solver=reutersidf_pt_solver.prototxt --weights=reutersidf_stack_init.caffemodel')


        net = caffe.Net('reutersidf_stack_net.prototxt', 'exp/'+db+'/save_iter_%d.caffemodel'%params['pt_iter'][0])
        w_down.append(net.params['d_'+str_x][0].data.copy())
        b_down.append(net.params['d_'+str_x][1].data.copy())
        del net

    net = caffe.Net('reutersidf_pt_net.prototxt', 'exp/'+db+'/save_iter_%d.caffemodel'%params['pt_iter'][0])
    for i in xrange(n_layer):
        if i == 0:
            k = 'd_data'
        else:
    ...
    net.save('reutersidf_stack_init_final.caffemodel')
...
if __name__ == '__main__':
    ...
    input_dim = 2000
    #dec.make_mnist_data()
    print main(db, {'n_layer':[5], 'dim': [input_dim, 500, 500, 1000, 2000, 4],
               'drop': [0.0], 'rate': [0.01], 'step': [20000], 'iter':[100000], 'decay': [0.0000]})
    print pretrain_main(db, {'dim': [input_dim, 500, 500, 1000, 2000, 4], 'pt_iter': [50000],
              'drop': [0.2], 'rate': [0.01], 'step': [20000], 'iter':[100000], 'decay': [0.0000]})
    os.system("caffe train --solver=reutersidf_ft_solver.prototxt --weights=reutersidf_stack_init_final.caffemodel")
